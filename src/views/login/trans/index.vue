  ...
  methods: {
    initAuthorize() {
      if (Misc.getQueryString('code')) {
        ...
        window.location.replace('https://open.weixin.qq.com/connect/oauth2/authorize?appid=' + this.appid + '&redirect_uri=' + encodeURIComponent(`http://c.jrj.com.cn/hotel-client.html#/auth`) + '&' + 'response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect')
      }
    },
    // 检查手机号是否绑定
